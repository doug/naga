...
func TestTypeRegistry_ScalarDeduplication(t *testing.T) {
	...
	// Register f32 twice
	f32_1 := registry.GetOrCreate("f32", ir.ScalarType{Kind: ir.ScalarFloat, Width: 4})
	f32_2 := registry.GetOrCreate("f32", ir.ScalarType{Kind: ir.ScalarFloat, Width: 4})
...
func TestTypeRegistry_DifferentScalars(t *testing.T) {
	...
	f32 := registry.GetOrCreate("f32", ir.ScalarType{Kind: ir.ScalarFloat, Width: 4})
	i32 := registry.GetOrCreate("i32", ir.ScalarType{Kind: ir.ScalarSint, Width: 4})
	u32 := registry.GetOrCreate("u32", ir.ScalarType{Kind: ir.ScalarUint, Width: 4})
	f16 := registry.GetOrCreate("f16", ir.ScalarType{Kind: ir.ScalarFloat, Width: 2})

	// All should be different
	handles := []ir.TypeHandle{f32, i32, u32, f16}
...
func TestTypeRegistry_VectorDeduplication(t *testing.T) {
	...
	vec4_1 := registry.GetOrCreate("", ir.VectorType{Size: ir.Vec4, Scalar: scalar})
	vec4_2 := registry.GetOrCreate("", ir.VectorType{Size: ir.Vec4, Scalar: scalar})
...
func TestTypeRegistry_DifferentVectors(t *testing.T) {
	...
	vec4f32 := registry.GetOrCreate("", ir.VectorType{Size: ir.Vec4, Scalar: f32})
	vec3f32 := registry.GetOrCreate("", ir.VectorType{Size: ir.Vec3, Scalar: f32})
	vec4i32 := registry.GetOrCreate("", ir.VectorType{Size: ir.Vec4, Scalar: i32})
...
func TestTypeRegistry_MatrixDeduplication(t *testing.T) {
	...
	scalar := ir.ScalarType{Kind: ir.ScalarFloat, Width: 4}
	mat4x4_1 := registry.GetOrCreate("", ir.MatrixType{Columns: ir.Vec4, Rows: ir.Vec4, Scalar: scalar})
	mat4x4_2 := registry.GetOrCreate("", ir.MatrixType{Columns: ir.Vec4, Rows: ir.Vec4, Scalar: scalar})
...
func TestTypeRegistry_DifferentMatrices(t *testing.T) {
	...
	mat4x4 := registry.GetOrCreate("", ir.MatrixType{Columns: ir.Vec4, Rows: ir.Vec4, Scalar: f32})
	mat3x3 := registry.GetOrCreate("", ir.MatrixType{Columns: ir.Vec3, Rows: ir.Vec3, Scalar: f32})
	mat4x3 := registry.GetOrCreate("", ir.MatrixType{Columns: ir.Vec4, Rows: ir.Vec3, Scalar: f32})
...
func TestTypeRegistry_ArrayDeduplication(t *testing.T) {
	...
	size := uint32(10)
	array1 := registry.GetOrCreate("", ir.ArrayType{
		Base:   f32,
		Size:   ir.ArraySize{Constant: &size},
		Stride: 4,
	})
	array2 := registry.GetOrCreate("", ir.ArrayType{
		Base:   f32,
		Size:   ir.ArraySize{Constant: &size},
		Stride: 4,
	})
...
func TestTypeRegistry_DifferentArrays(t *testing.T) {
	...
	arrayF32_10 := registry.GetOrCreate("", ir.ArrayType{
		Base:   f32,
		Size:   ir.ArraySize{Constant: &size10},
		Stride: 4,
	})
	arrayF32_20 := registry.GetOrCreate("", ir.ArrayType{
		Base:   f32,
		Size:   ir.ArraySize{Constant: &size20},
		Stride: 4,
	})
	arrayI32_10 := registry.GetOrCreate("", ir.ArrayType{
		Base:   i32,
		Size:   ir.ArraySize{Constant: &size10},
		Stride: 4,
	})
	arrayRuntime := registry.GetOrCreate("", ir.ArrayType{
		Base:   f32,
...
func TestTypeRegistry_StructDeduplication(t *testing.T) {
	...
	struct1 := registry.GetOrCreate("Vertex", ir.StructType{Members: members, Span: 32})
	struct2 := registry.GetOrCreate("Vertex", ir.StructType{Members: members, Span: 32})
...
func TestTypeRegistry_DifferentStructs(t *testing.T) {
	...
	struct1 := registry.GetOrCreate("Struct1", ir.StructType{
		Members: []ir.StructMember{
			{Name: "a", Type: f32, Offset: 0},
		},
		Span: 16,
	})
	struct2 := registry.GetOrCreate("Struct2", ir.StructType{
		Members: []ir.StructMember{
			{Name: "a", Type: i32, Offset: 0},
		},
		Span: 16,
	})
	struct3 := registry.GetOrCreate("Struct3", ir.StructType{
		Members: []ir.StructMember{
...
func TestTypeRegistry_PointerDeduplication(t *testing.T) {
	...
	ptr1 := registry.GetOrCreate("", ir.PointerType{Base: f32, Space: ir.SpaceFunction})
	ptr2 := registry.GetOrCreate("", ir.PointerType{Base: f32, Space: ir.SpaceFunction})
...
func TestTypeRegistry_DifferentPointers(t *testing.T) {
	...
	ptrFunc := registry.GetOrCreate("", ir.PointerType{Base: f32, Space: ir.SpaceFunction})
	ptrPriv := registry.GetOrCreate("", ir.PointerType{Base: f32, Space: ir.SpacePrivate})
	ptrStorage := registry.GetOrCreate("", ir.PointerType{Base: f32, Space: ir.SpaceStorage})
...
func TestTypeRegistry_ImageDeduplication(t *testing.T) {
	...
	img1 := registry.GetOrCreate("", ir.ImageType{
		Dim:          ir.Dim2D,
		Arrayed:      false,
		Class:        ir.ImageClassSampled,
		Multisampled: false,
	})
	img2 := registry.GetOrCreate("", ir.ImageType{
		Dim:          ir.Dim2D,
		Arrayed:      false,
		Class:        ir.ImageClassSampled,
		Multisampled: false,
...
func TestTypeRegistry_DifferentImages(t *testing.T) {
	...
	img2D := registry.GetOrCreate("", ir.ImageType{
		Dim:          ir.Dim2D,
		Arrayed:      false,
		Class:        ir.ImageClassSampled,
		Multisampled: false,
	})
	img3D := registry.GetOrCreate("", ir.ImageType{
		Dim:          ir.Dim3D,
		Arrayed:      false,
		Class:        ir.ImageClassSampled,
		Multisampled: false,
	})
	img2DArray := registry.GetOrCreate("", ir.ImageType{
		Dim:          ir.Dim2D,
		Arrayed:      true,
		Class:        ir.ImageClassSampled,
		Multisampled: false,
	})
	img2DMS := registry.GetOrCreate("", ir.ImageType{
		Dim:          ir.Dim2D,
		Arrayed:      false,
		Class:        ir.ImageClassSampled,
		Multisampled: true,
...
func TestTypeRegistry_SamplerDeduplication(t *testing.T) {
	...
	sampler1 := registry.GetOrCreate("", ir.SamplerType{Comparison: false})
	sampler2 := registry.GetOrCreate("", ir.SamplerType{Comparison: false})
...
func TestTypeRegistry_DifferentSamplers(t *testing.T) {
	...
	samplerReg := registry.GetOrCreate("", ir.SamplerType{Comparison: false})
	samplerComp := registry.GetOrCreate("", ir.SamplerType{Comparison: true})
...
func TestTypeRegistry_Lookup(t *testing.T) {
	...
	// Try invalid handle
	_, ok = registry.Lookup(ir.TypeHandle(999))
	if ok {
...
func TestTypeRegistry_ComplexNestedTypes(t *testing.T) {
	...
	_ = registry.GetOrCreate("f32", ir.ScalarType{Kind: ir.ScalarFloat, Width: 4})
	vec4f32 := registry.GetOrCreate("", ir.VectorType{Size: ir.Vec4, Scalar: ir.ScalarType{Kind: ir.ScalarFloat, Width: 4}})

	vertex := registry.GetOrCreate("Vertex", ir.StructType{
		Members: []ir.StructMember{
	...
	vertex2 := registry.GetOrCreate("Vertex", ir.StructType{
		Members: []ir.StructMember{
			{Name: "position", Type: vec4f32, Offset: 0},
			{Name: "color", Type: vec4f32, Offset: 16},
		},
