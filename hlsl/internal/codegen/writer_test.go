...
func TestWriter_WriteModule_Header(t *testing.T) {
	// Rust naga HLSL backend does not emit any header comments.
	// Verify that our output is clean (no header comments).
	module := &ir.Module{}
	opts := DefaultOptions()
	...
	// No header comments (matches Rust naga)
	if strings.Contains(output, "Generated by naga") {
		t.Error("should not contain header comment (Rust naga doesn't emit one)")
	}
}
...
func TestWriter_ScalarValue(t *testing.T) {
	...
	tests := []struct {
		...
		{"int positive", ir.ScalarValue{Bits: 42, Kind: ir.ScalarSint}, "int(42)"},
		{"int negative", ir.ScalarValue{Bits: 0xFFFFFFFF, Kind: ir.ScalarSint}, "int(-1)"},
